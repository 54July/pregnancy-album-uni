...
export const useChecklistStore = defineStore('checklist', {
  ...
  getters: {
    allItems(state) {
      return state.categories.flatMap((cat) => [...cat.items, ...cat.customItems])
    },
    checkedCount(state) {
  ...
  actions: {
    ...
    /** 合并 AI 解析的分类到当前清单（以 customItems 形式追加） */
    mergeAiCategories(aiCategories) {
      aiCategories.forEach((aiCat) => {
        // 找现有分类（名字模糊匹配）或追加到第一分类
        let target = this.categories.find((c) =>
          c.name.includes(aiCat.name) || aiCat.name.includes(c.name),
        )
        if (!target) target = this.categories[0]
        aiCat.items.forEach((itemName) => {
          const exists = [
            ...target.items,
            ...target.customItems,
          ].some((i) => i.name === itemName)
          if (!exists) {
            target.customItems.push({ id: createId(), name: itemName, checked: false })
          }
        })
      })
      saveState(this)
